...
- Types of logs:
	...
	- DB: Database logs. Audit trails that come from databases like MariaDB or Postgres.  
...
- Immutable: The inability to be mutated (changed) after creation.  
 ...
 - Structure of Logs:
	...
	- Systemd Journal: Binary log format used by `journald` and accessed with the
      `journalctl` command.  
        - Captures structured logs with metadata (UID, PID, etc).  

- Log rotation: The process of archiving old log files and starting new ones.  
    - This is done to save on disk space and ensure long-term availability of older
      ones.  
    - Managed by tools like `logrotate` (configured in `/etc/logrotate.conf` and
      files in `/etc/logrotate.d/`).  
 
- Rsyslog: An enhanced syslog implementation for linux.  
    - Capabe of handling structured data and filtering, as well as forwarding logs to
      remote servers. 
    - Very configurable via `/etc/rsyslog.conf` and files in `/etc/rsyslog.d`.  

- Log aggregation: The process of collecting and combining logs from multiple sources
  into a centralized platform for analysis, alerts, and retention.  
	- ELK: ElasticSearch, Logstash, Kibana. A popular open-source stack for log
      search and visualization.  
	- Splunk: Commercial log aggregation and SIEM tool.  
        - Easy to use, powerful dashboards, very expensive.  
	- Graylog: Open-source log collector and analyzer.  
        - Uses ElasticSearch under the hood.  
        - Balance between easy-of-use and flexibility.  
	- Loki: A log aggregation system from Grafana Labs.  
        - Acts as a TSDB.  
        - Optimized for metrics-style querying (like Prometheus).  
        - Often paired with Grafana.  

- SIEM: Security Information and Event Management
    - A system that collects, analyzes, and corelates security logs from across an
      environment.  
    - Combines log aggregation, alerting, correlation, and incident detection.  
    - Helps detect threats like brute force, privilege escalations, or lateral
      movement.  
    - Some SIEM tools are: Splunk, ELK with security plugins, IBM QRadar, Wazuh  
...
- Not technically my "career", but I use a monitoring stack in my homelab:
    - Prometheus + Node_Exporter (metrics aggregation and storage)
    - Loki + Promtail (log aggregation and storage)
